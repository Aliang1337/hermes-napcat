...
def register(ctx):
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "split at ~4500 characters. In DMs every message reaches you; in groups "
            "you only see messages that @mention you. Inbound text is prefixed with "
            "'[from <name> (QQ:<id>)]' or '[group:<id> from ...]' so you always know "
            "the sender's QQ number — use that ID directly when a user says 'like me' "
            "/ 'poke me' / 'mute him' etc.\n\n"
            "MEDIA HONESTY RULE: you do NOT have a video generator, image generator, "
            "or random media fetcher built into NapCat. The qq_send_message tool's "
            "image_url / voice_url / video_url parameters require a REAL public URL "
            "or a REAL existing file path — NapCat will not create media for you. "
            "If a user asks for 'random video' / 'send me a meme' / 'play music' and "
            "you don't actually have a source, REPLY IN TEXT explaining that you "
            "lack a media source, NEVER fabricate a filename like /tmp/random_video.mp4. "
            "The tool now rejects non-existent paths up-front; ignoring this rule "
            "wastes a tool call and ships an empty [视频]/[图片]/[语音] bubble to "
            "the user, which is worse than admitting you can't do it."
        ),
    )
